	...
	for (int i_param_combination = index; i_param_combination < num_combinations_current_batch; i_param_combination+=stride) {
		...
		for (int i_gene = 0; i_gene < num_genes; i_gene++) {
			...
			for (int i_count = 0; i_count < max_count; i_count++) {
				...
				log_likelihood += mrna_count[i_simulated_count] * log(real_distributions[i_real_count]);
			}
			
			for (int i_count = 0; i_count < max_count; i_count++ ) {
				i_real_count = i_gene * max_count + i_count;
				i_simulated_count = i_param_combination * max_count + i_count;
				mse += (initial_distributions[i_simulated_count] - real_distributions[i_real_count]) * (initial_distributions[i_simulated_count] - real_distributions[i_real_count]);
			}
...
// nvcc /home/data/nlaszik/cuda_simulation/code/cuda/create_and_fit_distributions_gillespie_profile.cu -o /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_gillespie_profile -lcurand -lboost_filesystem -lboost_system -lineinfo

// SRP215251
// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_gillespie_profile -mi 100000 -mt 50.0 -mc 400 -s 1.0 -h 4.0 -bs 2000000 -i /home/data/Shared/shared_datasets/sc-rna-seq/SRP215251/figures/wt/tpm_filtered.csv -o /home/data/nlaszik/cuda_simulation/output/SRP215251/WT -mode no_np -ll -5.0 -ul 1.0

// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_gillespie_profile -mi 100000 -mt 50.0 -mc 400 -s 1.0 -h 4.0 -bs 2000000 -i /home/data/Shared/shared_datasets/sc-rna-seq/SRP215251/figures/dko/tpm_filtered.csv -o /home/data/nlaszik/cuda_simulation/output/SRP215251/DKO -mode no_np -ll -5.0 -ul 1.0

// SRP313343
// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_gillespie_profile -mi 100000 -mt 50.0 -mc 400 -s 1.0 -h 4.0 -bs 2000000 -i /home/data/Shared/shared_datasets/sc-rna-seq/SRP313343/seurat/transcript_counts/srr14139729_transcript_counts.filtered.norm.csv -o /home/data/nlaszik/cuda_simulation/output/SRP313343/SRR14139729 -mode no_np -ll -5.0 -ul 1.0

// SRP299892
// smaller batch size due to higher # of cells?
// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_gillespie_profile -mi 100000 -mt 10.0 -mc 400 -s 0.1 -h 2.0 -bs 1000000 -p /home/data/nlaszik/cuda_simulation/output/SRP299892/no_np/time10_step0.05_h1_lower-3_upper3/initial_parameters.csv -i /home/data/Shared/shared_datasets/sc_rna_seq/data/SRP299892/seurat/transcript_counts/srr13336770_transcript_counts.filtered.norm.csv -o /home/data/nlaszik/cuda_simulation/output/SRP299892 -mode no_np -ll -3.0 -ul 3.0

// DKO
// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_gillespie_profile -mi 100000 -mt 10.0 -mc 150 -s 0.05 -h 1.0 -bs 1000000 -i /home/data/Shared/shared_datasets/sc_rna_seq/dko_atrinh_102022/seurat/transcript_counts/rep2_transcript_counts.filtered.norm.csv -o /home/data/nlaszik/cuda_simulation/output/dko_hesc/rep2 -mode no_np -ll -3.0 -ul 3.0

// DKO same median
// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_gillespie_profile -mi 100000 -mt 50.0 -mc 150 -s 1.0 -h 4.0 -bs 1000000 -i /home/data/Shared/shared_datasets/sc-rna-seq/dko_hesc/seurat/set_median/rep1_transcript_counts.filtered.norm.csv -o /home/data/nlaszik/cuda_simulation/output/dko_hesc/rep1_gillespie_set_median/ -mode no_np -ll -5.0 -ul 1.0

// Andrew's data
// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_gillespie_profile -mi 100000 -mt 50.0 -mc 400 -s 1.0 -h 4.0 -bs 1000000 -i /home/data/Shared/AQPhan/norm_counts.csv -o /home/data/nlaszik/cuda_simulation/output/aqphan/ -mode no_np -ll -5.0 -ul 1.0

// SRP364225
// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_gillespie_profile -mi 100000 -mt 50.0 -mc 400 -s 1.0 -h 4.0 -bs 1000000 -i /home/data/Shared/shared_datasets/sc-rna-seq/SRP364225/seurat/SRR18335026_transcript_counts.filtered.norm.csv -o /home/data/nlaszik/cuda_simulation/output/SRP364225/SRR18335026_norm/ -mode no_np -ll -5.0 -ul 1.0
...
int main(int argc, char** argv)
{
	...
	for (int i_gene = 0; i_gene < param_gene_names.size(); i_gene++) {
		...
		if(itr != gene_names.end()) {
			int i_common = std::distance(v.begin(), itr);
	...
	vector<vector<vector<double>>> param_combinations_vector(num_genes);
		...
		for (int i_param = 0; i_param < num_params; i_param++){
			if (param_lower_limits[i_param] != -DBL_MAX && param_lower_limits[i_param] != DBL_MAX){
				for (double param = param_lower_limits[i_param]; param <= param_upper_limits[i_param]; param += step_sizes[i_param]){
					...
					param_combinations_vector[i_gene].push_back(params_vector[i_gene]);
					
					if (i_param == 4){
						// p_np is a probability, not a rate
						param_combinations_vector[i_gene][i_combination][i_param] = param;
					}
					else {
						// rate, so use log
						param_combinations_vector[i_param][i_combination][i_param] = pow(10.0, param);
					}
					i_combination++;
				}
			}
			else if (param_lower_limits[i_param] == DBL_MAX){
				param_combinations_vector[i_gene].push_back(params_vector[i_gene]); // this is the starting point from params
				param_combinations_vector[i_param][i_combination][i_param] = DBL_MAX;
				i_combination++;
			}
			else {
				param_combinations_vector[i_gene].push_back(params_vector[i_gene]); // this is the starting point from params
				param_combinations_vector[i_param][i_combination][i_param] = 0.0;
				i_combination++;
			}
		}
		i_combination = 0;
	...
	cudaMallocManaged(&mrna_count, num_genes * num_cells * sizeof(int));
	cudaMallocManaged(&initial_distributions, num_genes * max_count * sizeof(double));
	cudaMallocManaged(&param_combinations, num_genes * num_params * sizeof(double));
