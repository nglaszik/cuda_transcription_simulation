	...
	for (int i_param_combination = index; i_param_combination < num_combinations_current_batch; i_param_combination+=stride) {
		...
		for (int i_cell = 0; i_cell < num_cells; i_cell++) {
			...
			if (r_np < param_combinations[i_param_combination * num_params + 4]) {
				...
				while (time < max_time && mrna_count[i_cell_param_combination] < max_count - 1 && (transcriptional_states[i_cell_param_combination] < 2 || mrna_count[i_cell_param_combination] > 0)) {
...
int main(int argc, char** argv)
{
	...
	// inputs should be:
	// 1. gene length
	// 2. exon locations and sizes
	// 3. polymerase elongation rate at introns
	// 4. polymerase elongation rate at exons
	// 5. polymerase initiation rate
	
	// can get rid of:
	// k_on & k_off
	
	// do we randomly generate all these? perhaps
	// the additional two rates should be easy to do
	// randomly generating exon locations and gene lengths is difficult... maybe we can load these for particular genes? we have real data
	// so that information will be set, takes care of a lot of parameter space
	
	// simulation:
	// 1. need to track polymerase locations along gene... data struct should be max gene length divided by minimum movement rate?
	// gene x cell x 
	// actual speed of polymerase is median 1.5kb/min... maybe we can bin this somehow... at each tick, a polymerase can move 100/500/etc. bps. otherwise it's quite difficult to model this
	// some initial guesses would be 10kb/min at introns, 0.5kb/min at exons?
	// once a polymerase gets to the end of a gene, we can increment mrna count
	// a polymerase must check to see if there is a polymerase occupying the current position ahead of it
	// the "movement rate" doesn't need to reflect how far a polymerase moves with each tick, it's more like the propensity for movement of one tick distance
	
	// questions this may answer:
	// does the inferred k_i matches up well with the accessibility at the TSS?
	// does the inferred k_e_exon match up with methylation at exons, or methylation density?
	
	// maybe to start with we just look at different equally spaced exon densities and sizes for a gene of the same length... can we even create burstiness?
	// introduces 4 new parameters:
	// number of exons, density of exons, size of exons, gene lengths
	...
	int param_lower_limits[num_params] = {int_lower_limit, 		int_lower_limit,	int_lower_limit, (int)(k_deg * multiplier)};
	int param_upper_limits[num_params] = {int_upper_limit, 		int_upper_limit,	int_upper_limit, (int)(k_deg * multiplier)};
