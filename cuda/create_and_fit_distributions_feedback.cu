	...
	for (int i_param_combination = index; i_param_combination < num_combinations_current_batch; i_param_combination+=stride) {
		...
		for (int i_cell = 0; i_cell < num_cells; i_cell++) {
			...
			if (r_age < param_combinations[i_param_combination * num_params + 3]) {
				...
				while (timestep < num_timesteps && mrna_count[i_cell_param_combination] < max_count) {
					...
					int r_methylation = (int)(ceil(generate(globalState, i_param_combination)*(num_timesteps*100 + 1)) - 1); // random uniform number, around 1% of genes will methylate over timecourse if p=1
					...
					if (transcriptional_states[i_cell_param_combination] < 2){
						if (r_methylation < param_combinations[i_param_combination * num_params + 2]) {
							transcriptional_states[i_cell_param_combination] = 2;
						}
					}
					...
					if (transcriptional_states[i_cell_param_combination] < 2) {
						...
						int r_switch = (int)(ceil(generate(globalState, i_param_combination)*(1000 + 1)) - 1); //random uniform number, 0-1000
						if (transcriptional_states[i_cell_param_combination] == 0) {
							if (r_switch < param_combinations[i_param_combination * num_params + 0]) {
								transcriptional_states[i_cell_param_combination] = 1;
							}
						} else {
							if (r_switch < param_combinations[i_param_combination * num_params + 1]) {
								transcriptional_states[i_cell_param_combination] = 0;
							} else {
								//printf("assessing transcription %i, cell %i, total index %i\n", i_param_combination, i_cell, i_cell_param_combination);
								int r_transcription = (int)(ceil(generate(globalState, i_param_combination)*(1040 + 1)) - 1); //random uniform number, 0-1000
								int p_feedback_transcription = (int)((double)param_combinations[i_param_combination * num_params + 4] * ((double)1.0 + (double)mrna_count[i_cell_param_combination])/(double)param_combinations[i_param_combination * num_params + 5]);
								if (r_transcription < p_feedback_transcription) {
									//printf("transcription occurred\n");
									// param 5 is the characteristic incrementing threshold...
...
// nvcc /home/data/nlaszik/cuda_simulation/code/cuda/create_and_fit_distributions_feedback.cu -o /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_feedback -lcurand

// SRP215251
// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_feedback -nt 5000 -mc 400 -s 100 -h 4.0 -bs 2000000 -i /home/data/Shared/shared_datasets/sc-rna-seq/SRP215251/figures/wt/tpm_filtered.csv -o /home/data/nlaszik/cuda_simulation/output/SRP215251/WT_feedback/

// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_feedback -nt 5000 -mc 400 -s 100 -h 4.0 -bs 2000000 -i /home/data/Shared/shared_datasets/sc-rna-seq/SRP215251/figures/dko/tpm_filtered.csv -o /home/data/nlaszik/cuda_simulation/output/SRP215251/DKO_feedback/

// SRP313343
// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_feedback -nt 5000 -mc 400 -s 100 -h 4.0 -bs 2000000 -i /home/data/Shared/shared_datasets/sc-rna-seq/SRP313343/seurat/transcript_counts/srr14139729_transcript_counts.filtered.norm.csv -o /home/data/nlaszik/cuda_simulation/output/SRP313343/SRR14139729_norm_feedback/

// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_feedback -nt 5000 -mc 400 -s 100 -h 4.0 -bs 2000000 -i /home/data/Shared/shared_datasets/sc-rna-seq/SRP313343/seurat/transcript_counts/srr14139730_transcript_counts.filtered.norm.csv -o /home/data/nlaszik/cuda_simulation/output/SRP313343/SRR14139730_norm_feedback/

// SRP299892
// smaller batch size due to higher # of cells?
// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_feedback -nt 5000 -mc 400 -s 100 -h 4.0 -bs 1000000 -i /home/data/Shared/shared_datasets/sc-rna-seq/SRP299892/seurat/transcript_counts/srr13336770_transcript_counts.filtered.norm.csv -o /home/data/nlaszik/cuda_simulation/output/SRP299892/SRR13336770_norm_feedback/

// Andrew's data
// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_feedback -nt 5000 -mc 400 -s 100 -h 4.0 -bs 1000000 -i /home/data/Shared/AQPhan/norm_counts.csv -o /home/data/nlaszik/cuda_simulation/output/aqphan_feedback/


// SRP364225
// /home/data/nlaszik/cuda_simulation/code/cuda/build/create_and_fit_distributions_feedback -nt 5000 -mc 400 -s 100 -h 4.0 -bs 1000000 -i /home/data/Shared/shared_datasets/sc-rna-seq/SRP364225/seurat/SRR18335026_transcript_counts.filtered.norm.csv -o /home/data/nlaszik/cuda_simulation/output/SRP364225/SRR18335026_norm_feedback/
...
int main(int argc, char** argv)
{
	...
	// f is the characteristic number of mrnas that doubles the transcription rate
	
	int param_lower_limits[num_params] = {0, 0, 0, 0, 0, 10000}; // the max p we can get is 2, so this number cannot be lower than max_count... feedback ranges from 4% to .04%
	int param_middle_limits[num_params] = {200, 600, 1000, 1000, 1000, 100000};
	int param_upper_limits[num_params] = {1000, 1000, 1000, 1000, 1000, 100000}; // top f should represent as little feedback as possible, pick some high number. 4k gives 10% feedback at 400 count
	
	int lower_step_sizes[num_params] = {(int)(step/20), (int)(step/2), (int)(step/2), (int)(step/2), (int)(step/2), 18000};
	int higher_step_sizes[num_params] = {(int)(step/2), (int)(step/4), (int)(step/2), (int)(step/2), (int)(step/2), 18000};
	...
	printf("%s\n", path_parameters);
	fprintf(outfile_parameters, "gene,on,off,np,p_np,tx,f,\n");
	for (int i_gene = 0; i_gene < num_genes; i_gene++){
		fprintf(outfile, "%s,", gene_names[i_gene].c_str());
	    for (int i_param = 0; i_param < num_params; i_param++){
