...
__global__
void find_initial_best_fit(int num_genes, int num_cells, int batch_size, int num_combinations_in_batch, int i_batch, int max_count, int kde_granularity, double *simulated_distributions, double *real_distributions, double *best_likelihoods, int *best_params){
...
// nvcc /home/data/nlaszik/cuda_simulation/code/cuda/fit_distributions_ll.cu -o /home/data/nlaszik/cuda_simulation/code/cuda/build/fit_distributions_ll -lcurand -lboost_filesystem -lboost_system -lineinfo

// /home/data/nlaszik/cuda_simulation/code/cuda/build/fit_distributions_ll -bs 1000000 -i /home/data/Shared/shared_datasets/sc_rna_seq/data/SRP299892/seurat/transcript_counts/srr13336770_transcript_counts.filtered.norm.csv -d /home/data/nlaszik/cuda_simulation/output/simulated/no_np/max400_time10_step0.1_h1_lower-3_upper3_deg0 -o /home/data/nlaszik/cuda_simulation/output/SRP299892_ll
...
// /home/data/nlaszik/cuda_simulation/code/cuda/build/fit_distributions_ll -bs 1000000 -i /home/data/Shared/shared_datasets/sc_rna_seq/data/SRP299892/seurat/transcript_counts/srr13336770_transcript_counts.filtered.norm.csv -d /home/data/nlaszik/cuda_simulation/output/simulated_methylation_new/k_tx/max400_time10_step0.25_h2_lower-3_upper3_deg0 -o /home/data/nlaszik/cuda_simulation/output/SRP299892_ll

// /home/data/nlaszik/cuda_simulation/code/cuda/build/fit_distributions_ll -bs 1000000 -i /home/data/Shared/shared_datasets/sc_rna_seq/nlaszik/dko_atrinh_102022/seurat/transcript_counts/rep2_transcript_counts.filtered.norm.csv -d /home/data/nlaszik/cuda_simulation/output/simulated/no_np/max400_time1000_step0.1_h2_lower-3_upper3_deg0 -o /home/data/nlaszik/cuda_simulation/output/dko_hesc_ll
...
int main(int argc, char** argv)
{
	...
	tie(path_output_counts, path_output_parameters, path_output_likelihoods, path_simulated_counts, path_simulated_params, path_simulated_kdes) = run_path_checks(path_indir, path_outdir, max_count, max_time, step, h, lower_limit, upper_limit, k_deg, path_mode);
	...
	for (int i_batch = 0; i_batch < num_batches; i_batch++){
		...
		for (int i_batch_combination = i_batch * batch_size; i_batch_combination < (i_batch + 1) * batch_size; i_batch_combination++){
			if (i_batch_combination < num_param_combinations){
				double sum_total = 0.0;
				for (int i_x = 0; i_x < max_count * kde_granularity; i_x++){
					int i_dist = i_combination_in_batch * max_count * kde_granularity + i_x;
		...
		find_initial_best_fit<<<numBlocks, blockSize>>>(num_genes, num_cells, batch_size, num_combinations_in_batch, i_batch, max_count, kde_granularity, simulated_distributions, real_distributions, best_likelihoods, best_params);
